...
% comm_r: communication radius
% anchor_comm_r: The communication radius of the anchor node is a multiple of the 
% communication radius of the unknown node
% model: the selected communication model
% DOI: Irregularity, valid when model is'DOI Model','RIM Model', otherwise it can be defaulted
    ...
    try
        for i=1:all_nodes.nodes_n
            dist=sqrt(sum(transpose((repmat(all_nodes.true(i,:),all_nodes.nodes_n-1,1) ...
                -all_nodes.true([1:i-1,i+1:all_nodes.nodes_n],:)).^2)));
            RSS=dist2rss(dist);
